        ...
        try:
            ...
            if self._sort_field in self._primary_key:
                ...
                self._record_transformation(
                    ...
                    f"df.sort_values({repr(self._sort_field)}, ascending={self._sort_ascending}, "
                    f"inplace=True)"
                )
                self._disable_grouping = True
        except TypeError:
            ...
            self._record_transformation(
                (
                    ...
                    f"df.sort_values({repr(helper_col)}, ascending={self._sort_ascending}, inplace=True)\n"
                    f"df.drop(columns={repr(helper_col)}, inplace=True)"
                )
            )
        ...
        if filter_info["type"] == "slider":
            if filter_info["min"] is not None:
                ...
                self._filter_conditions.append(
                    f"unfiltered_df.loc[:,{repr(col_name)}] >= {filter_info['min']}"
                )
            if filter_info["max"] is not None:
                conditions.append(col_series <= filter_info["max"])
                self._filter_conditions.append(
                    f"unfiltered_df.loc[:,{repr(col_name)}] <= {filter_info['max']}"
                )
            ...
            if filter_info["min"] is not None:
                ...
                self._filter_conditions.append(
                    f"unfiltered_df.loc[:,{repr(col_name)}] >= pd.to_datetime({filter_info['min']}, unit='ms')"
                )
            if filter_info["max"] is not None:
                conditions.append(
                ...
                self._filter_conditions.append(
                    f"unfiltered_df.loc[:,{repr(col_name)}] <= pd.to_datetime({filter_info['max']}, unit='ms')"
                )
        elif filter_info["type"] == "boolean":
            if filter_info["selected"] is not None:
                conditions.append(col_series == filter_info["selected"])
                self._filter_conditions.append(
                    f"unfiltered_df.loc[:,{repr(col_name)}] == {filter_info['selected']}"
                )
            ...
            if selected_indices == "all":
                if excluded_indices is not None and len(excluded_indices) > 0:
                    ...
                    self._filter_conditions.append(
                        f"~unfiltered_df.loc[:,{repr(col_name)}].isin({excluded_values})"
                    )
                ...
                self._filter_conditions.append(
                    f"unfiltered_df.loc[:,{repr(col_name)}].isin({selected_values})"
                )
